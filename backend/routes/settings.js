...
router.put('/settings', async (req, res) => {
  ...
  if (totalLocker >= maxLockerId) {
    const [result] = await db.query("UPDATE totalLocker SET total = ?", [totalLocker]);

    if (result.affectedRows === 0) {
      return res.json({ message: "No row found to update.", updated: false });
    }

    if (result.changedRows === 0) {
      return res.json({ message: "Total value is already the same. No changes made.", updated: false });
    }

    return res.json({ message: "Total locker updated successfully.", updated: true, affectedRows: result.affectedRows });
  } else {
    return res.json({
...
router.post("/insert-user", async (req, res) => {
  ...
  try {
    const [existing] = await db.query("SELECT * FROM users WHERE tupcID = ?", [tupcID]);

    if (existing.length > 0) {
      return res.status(409).json({ message: "User already exists." });
    }

    await db.query("INSERT INTO users (tupcID, balance) VALUES (?, ?)", [tupcID, balance]);
...
router.put('/deactivate-locker/:id', async (req, res) => {
  ...
  try {
    ...
    if (result.affectedRows === 0) {
      return res.status(404).json({ message: "Locker not found or already inactive." });
    }

    res.json({ message: "Locker deactivated successfully." });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Database error while deactivating locker." });
  }
});
